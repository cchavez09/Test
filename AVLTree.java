...
class LUC_AVLTree {
    ...
    private Node deleteElement(int value, Node node) {
      ...
      // if tree isn't empty, check if the value is less than the current node's value
      // or greater than
      if (node.value < value) {
        ...
        // check whether node we want to delete has children
        if ((node.leftChild == null) && (node.rightChild == null)) {
          node = null;
          // check whether it has a right child, move right child up
        } else if ((node.leftChild == null) && (node.rightChild != null)) {
          node = node.rightChild;
          // check whether it has left child, move child up
        } else if ((node.leftChild != null) && (node.rightChild == null)) {
          node = node.leftChild;
        } else {
          // if has both children, go to rightChild and left most successor
        ...
        // balance factor is checked at the end of this routine
        int balance = getBalanceFactor(node);
        if (balance > 1 && getBalanceFactor(node.leftChild) >= 0) {
          // if left left heavy, do LL rotation
          node = LLRotation(node);
        } else if (balance > 1 && getBalanceFactor(node.leftChild) < 0) {
          // if left right heavy, do LR rotation
          node = LRRotation(node);
        } else if (balance < -1 && getBalanceFactor(node.rightChild) <= 0) {
          // if right right heavy, do RR rotation
          node = RRRotation(node);
        } else if (balance < -1 && getBalanceFactor(node.rightChild) > 0) {
          // if right left heavy, do RL rotation
          node = RLRotation(node);
        }
